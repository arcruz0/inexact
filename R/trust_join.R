...
trust_join <- function (x, y, by, max_dist = Inf, method = c("osa", "lv", "dl", 
  ...
  res <- stringdist_join(x = x, y = y, by = by, max_dist = max_dist, 
  ...
  if (!is.null(custom_match)) {
    ...
    df_custom_match <- df_custom_match %>% mutate(.dist2 = -1)
    names(df_custom_match) <- c(quo_name(var_by_x), quo_name(var_by_y), 
                                ".dist2")
    suppressMessages({
      suppressWarnings({
        res <- res %>% left_join(df_custom_match)
      })
    })
    res <- res %>% replace_na(list(.dist2 = Inf)) %>% rowwise() %>% 
      mutate(.dist = min(.dist, .dist2)) %>% ungroup() %>% 
      select(-.dist2)
  }
  res <- res %>% group_by(!!var_by_x) %>% arrange(.dist) %>% 
    slice(1) %>% ungroup() %>% slice(match(v_order, .[[quo_name(var_by_x)]]))

  # names(res)[1] <- quo_name(var_by)
  names(res)[which(names(res) == quo_name(var_by_x))] <- quo_name(var_by)

  res[[quo_name(var_match)]] <- res[[quo_name(var_by_y)]]
  
  # res[[quo_name(var_by_x)]] <- NULL
  res[[quo_name(var_by_y)]] <- NULL
  ...
  res <- res %>% 
    select(unique(c(colnames(x), colnames(y))))
